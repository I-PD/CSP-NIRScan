...
namespace ISC_Win_WinForm_GUI
{
    public partial class MainWindow : Form
    {
        ...
        private Int32 TargetCfg_SelIndex = -1;       // Record device selected config
        private Int32 TargetCfg_Last_SelIndex = -1;  // Record last device selected config
        ...
        private Int32 LocalCfg_Last_SelIndex = -1;   // Record last local selected config
        ...
        public MainWindow(string[] args, string jwtToken, string username)
        {
            ...
            {
                try { await Editor_StartAsync(); }
                catch (Exception ex) { MessageBox.Show(ex.ToString(), "Editor init error"); }
            };
        }
        ...
        public void CommonUISetup(string[] args)
        {
            ...
            dataGridView_Table.KeyDown += dataGridView_Table_KeyDown;
            dataGridView_Editor.KeyDown += dataGridView_Editor_KeyDown; 

            formResize = new formResize(this);
            this.Load += Form_Load;
            //this.SizeChanged += Form_Resize; REMOVIDO PARA EVITAR PROBLEMAS NO RESIZE
            ...
            RadioButton_Reference.CheckedChanged += RadioButton_SpectrumData_CheckedChanged;
            RadioButton_Intensity.CheckedChanged += RadioButton_SpectrumData_CheckedChanged;
            RadioButton_Absorbance.CheckedChanged += RadioButton_SpectrumData_CheckedChanged;
            RadioButton_Reflectance.CheckedChanged += RadioButton_SpectrumData_CheckedChanged;

            pictureBox_heatMap.SizeMode = PictureBoxSizeMode.Zoom; // simple zoom fit
            pictureBox_heatMap.MouseWheel += pictureBox_heatMap_MouseWheel;
            pictureBox_heatMap.MouseEnter += (s, _) => pictureBox_heatMap.Focus(); // ensure it gets wheel events

            // === ligar handlers para detetar mudança no Material da tabela ===
            dataGridView_Table.CellValueChanged += (s, e) =>
            {
                if (e.RowIndex < 0) return;
                if (dataGridView_Table.Columns[e.ColumnIndex].Name != "Material") return;
                ToggleGrapeCaliberPanel();
            };
            // commits imediatos quando a célula é ComboBox
            dataGridView_Table.CurrentCellDirtyStateChanged += (s, e) =>
            {
                if (dataGridView_Table.IsCurrentCellDirty)
                    dataGridView_Table.CommitEdit(DataGridViewDataErrorContexts.Commit);
            };

            // avaliar já no arranque
            ToggleGrapeCaliberPanel();
        ...
        private void MainWindow_ResizeEnd(object sender, EventArgs e)
        {
            ...
            reqSizeChange = true;
            ResumeLayout();
            //(ver depois)label_ContinuousMode.Font = new Font(label_ContinueScan.Font.FontFamily, label_ContinueScan.Font.Size * 2, ((System.Drawing.FontStyle)(System.Drawing.FontStyle.Bold | System.Drawing.FontStyle.Italic)));
        }
        private void initBackgroundWorker()
        {
            ...
            ProgressBarForm.UserCancelRequest += new Action(() => { UserCancelScan = true; });
        ...
        private void InitGUIItem()
        {
            ...
            //CheckBox_SaveDAT.Checked = true; //opção .dat não é necessária de momento
        }
        #region Error 
        ...
        {
            _ = Device_Connected_Handler_WorkerAsync(serialNumber);
        }

        private async Task Device_Connected_Handler_WorkerAsync(string SerialNumber)
        {
            ...
            // UI: mostrar progress + limpar UI
            BeginInvoke((Action)(() =>
            {
                ProgressWindowStart("Device Open", "Connecting to the device...\r\nPlease wait!", false);

                // Move o teu bloco "Clear old information" para aqui (labels/textboxes = "")
            ...
            await Task.Run(() =>
            {
                try
                {
                    // >>>> Aqui entra o teu "try { do { ... } while(false); Thread.Sleep... }"
                    // MAS: NADA de mexer em UI diretamente aqui.

                    // Se precisares de chamar métodos que mexem em UI (ex: PopulateCfgDetailItems),
                    // chama-os via Invoke:
                    this.Invoke((Action)(() =>
                    {
                        // UI-only calls
                    ...
                    // device/SDK heavy calls (ficam no background)
                    GetActivationKeyStatus();
                    AutoSetKey();
                    GetDeviceInfo();
                    DeviceConnectBackUpRef();

                    // se RefreshErrorStatus mexe em UI, põe em Invoke também:
                    this.Invoke((Action)(() => RefreshErrorStatus()));

                    // resto do teu fluxo...
                    Thread.Sleep(SDK.ConnectionCheckInterval);
        ...
        //        //(add label_ref)label_refCalVer.Text = "";
        ...
        //        {
        //            Message.ShowError(FetchingDevInfoErrMsg, "Config Error", this);
        //            tabScanPositions.SelectedIndex = 1;
        ...
        private void UI_Setting_Connected()
        {
            ...
            UI_On_Connection(); //Quando conectado, UI ficará open para User trabalhar 
        ...
        private void BeginScan()
        {
            UI(() =>
            {
                int remaining = TargetScanCounts - ScannedCounts;

                if (ScanButtonPressed)
                {
                    bool cancellable = remaining != 1;
                    ProgressWindowStart("Scan Button Pressed", "Scan in progress... Please wait!", cancellable);
                    ScanButtonPressed = false;
                    return;
                }

                if (ReferenceSelect == Scan.SCAN_REF_TYPE.SCAN_REF_NEW || remaining == 1)
                {
        ...
        private void ScanCompleted()
        {
            UI(() =>
            {
                ...
                {
                    // Mantém a janela aberta em continuous scan; opcionalmente atualiza texto
                    // ProgressWindowContentUpdate($"Completed {ScannedCounts}/{TargetScanCounts}...");
                    return;
                }
        ...
        private void Button_CfgSave_Click(object sender, EventArgs e)
        {
            ...
            if (DevCurCfg_IsTarget == SelCfg_IsTarget && DevCurCfg_Index == (SelCfg_IsTarget ? TargetCfg_SelIndex : LocalCfg_SelIndex))
            {
                ...
                tabScanPositions.SelectedIndex = 0;
            }
            else
            {
        ...
        private void dataGridView_savescan_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            else if (hitSelectedCell.Type == DataGridViewHitTestType.Cell)
            {
                ...
                if (item.Contains(".csv") && Scan.ReadScanResultFromCsvFile(FileName) == SDK.RETURN_FAIL)
                {
                ...
                if (rb == RadioButton_Reference)
                    ShowPlot(ChartData_RefIntensity);
                else if (rb == RadioButton_Intensity)
                    ShowPlot(ChartData_Intensity);
                else if (rb == RadioButton_Absorbance)
                    ShowPlot(ChartData_Absorbance);
                else // Reflectance
                    ShowPlot(ChartData_Reflectance);
        ...
        private void SavedScan_ConvertToCSV()
        {
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                for (int i = 0; i < selFile.Count; i++)
                {
                    ...
                    string d = CSV_Delimiter;
                    for (int j = 0; j < Scan.ScanDataLen; j++)
                    {
                        var wl = Scan.WaveLength[j].ToString(CultureInfo.InvariantCulture);
                        var abs = Scan.Absorbance[j].ToString(CultureInfo.InvariantCulture);
                        var @ref = Scan.ReferenceIntensity[j].ToString(CultureInfo.InvariantCulture);
                        var sam = Scan.Intensity[j].ToString(CultureInfo.InvariantCulture);

                        sw.WriteLine(string.Join(d, wl, abs, @ref, sam));
                    }
                    /*
                    for (int j = 0; j < Scan.ScanDataLen; j++)
                    {
        ...
        {
            if (tabScanPositions.SelectedIndex == 2 && !IsSavedScanData)
                return;
            ...
            if (tabScanPositions.SelectedIndex == 2 && !Check_Overlay.Checked) // Save scans tab selected
            {
            ...
            else if (tabScanPositions.SelectedIndex == 2 && Check_Overlay.Checked)
            {
        ...
        private void SaveToFiles(string position)
        {
            ...
            if (CheckBox_FileNamePrefix.Checked == true)
            {
                ...
                CurrentScanFileName = Path.Combine(Dir_Scan_For_New, combinedPrefix + position/*Scan.ScanConfigData.head.config_name*/ + "_" + TimeScanStart.ToString("yyyyMMdd_HHmmss"));
                //CurrentScanFileName = @"\\?\" + CurrentScanFileName;
            }
            else
            {
                CurrentScanFileName = Path.Combine(Dir_Scan_For_New, position/*Scan.ScanConfigData.head.config_name*/ + "_" + TimeScanStart.ToString("yyyyMMdd_HHmmss"));
            }
            ...
            if (!Directory.Exists(dirpath))
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    {
                        Message.ShowError("Create directory failed!");
                        DBG.WriteLine(e.Message);
                        logFile.Error(e.Message);
                        return;
                    };
            ...
            // Se houve erro no dispositivo, marca isso no nome
            if (Device.ErrStatus > 0) CurrentScanFileName += "_Error_Detected";

            // Agora montamos o nome final do ficheiro, incluindo extensão
            string filePath = CurrentScanFileName + ".csv";

            // Se o ficheiro já existir, tenta apagá-lo; se falhar, acrescenta sufixo
            if (System.IO.File.Exists(filePath))
            {
                ...
                {
                    // adiciona "_1" antes da extensão
                    string dir = System.IO.Path.GetDirectoryName(filePath);
                    string name = System.IO.Path.GetFileNameWithoutExtension(filePath);
                    filePath = System.IO.Path.Combine(dir, name + "_1.csv");
                }
            }

            // Finalmente salva o CSV no caminho correcto
            SaveToCSV(filePath);
            if (CheckBox_AverageCSV.Checked) SaveToAverageCSV(filePath);
                    ...
                    using (FileStream fs = new FileStream(@FileName_one, FileMode.Append, FileAccess.Write))
                    {
                        using (StreamWriter sw = new StreamWriter(fs, System.Text.Encoding.UTF8))
                        {
                            if (fs.Length == 0)
                            {
                                ...
                                if (Device.ErrStatus == 0)  // Skip invalid scan data if error status received
                                {
                                    sw.WriteLine("Wavelength (nm)" + CSV_Delimiter);
                                    for (Int32 i = 0; i < Scan.ScanDataLen; i++)
                                        sw.WriteLine(Scan.WaveLength[i].ToString(CultureInfo.InvariantCulture) + CSV_Delimiter);
                                    sw.WriteLine("\n");

                                    sw.WriteLine("Reference Signal (unitless)" + CSV_Delimiter);
                                    for (Int32 i = 0; i < Scan.ScanDataLen; i++)
                                        sw.WriteLine(Scan.ReferenceIntensity[i].ToString(CultureInfo.InvariantCulture) + CSV_Delimiter);
                                    sw.WriteLine("\n");
                                }
                            }
                            ...
                            if (Device.ErrStatus == 0)  // Skip invalid scan data if error status received
                            {
                                sw.WriteLine("Sample Signal (unitless)" + CSV_Delimiter);
                                for (Int32 i = 0; i < Scan.ScanDataLen; i++)
                                    sw.WriteLine(Scan.Intensity[i].ToString(CultureInfo.InvariantCulture) + CSV_Delimiter);
                                sw.WriteLine("\n");
                            }
                        }
                    }
        ...
        private void bwScan_DoSacnCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (result == SDK.RETURN_PASS)
            {
                ...
                /* if (TargetScanCounts - ScannedCounts == 0)
                 ...
                 //(ver depois)Label_ContScan.Text = "(" + ScannedCounts.ToString() + "/" + TargetScanCounts.ToString() + ")";*/
                ...
                if ((TargetScanCounts - ScannedCounts) > 0 && Device.ErrStatus != 0)//&& checkBox_StopOnError.Checked )
                {
                    UserCancelScan = true;
                    SDK.IsConnectionChecking = true;
                    ProgressWindowCompleted();
                ...
                else
                {
                    ...
                    if ((TargetScanCounts - ScannedCounts) > 0)
                    {
                        ...
                        {
                            tabScanPositions.TabPages[0].Enabled = false;
                            tabScanPositions.TabPages[1].Enabled = false;
        ...
        private void Button_Cal_ReadCoeffs_Click(object sender, EventArgs e)
        {
            if (Device.GetCalibStruct() == SDK.RETURN_PASS)
            {
                ...
                //(add label_ref)label_refCalVer.Text = Device.DevInfo.RefCalRev.ToString();
            ...
            else
            {
                ...
                //(add label_ref)label_refCalVer.Text = "0";
        ...
        private void Button_SaveDirChange_Click(object sender, EventArgs e)
        {
            ...
            // secure ACLs so your user can write there
            try { AddDirectorySecurity(Dir_Scan_For_New); }
        ...
        {
            if (string.IsNullOrWhiteSpace(Dir_Scan_For_New) ||
                !Directory.Exists(Dir_Scan_For_New))
            {
                MessageBox.Show(
                    "Scan directory not found; resetting to default.",
                    "Directory Missing",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);

                // fallback once to LocalAppData\InnoSpectra\Scan Results
                var baseAppData = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
                Dir_Scan_For_New = Path.Combine(baseAppData, "InnoSpectra", "Scan Results");
                Directory.CreateDirectory(Dir_Scan_For_New);
        ...
        {
            switch (tabScanPositions.SelectedIndex)
            {
                case 0:
                    {
            ...
            lastTabScanPageSelection = tabScanPositions.SelectedIndex;
        }
        ...
        {
            if (!(tabScanPositions.SelectedIndex == 0))//RadioButton_RefNew.Checked && 
            {
                if (tabScanPositions.SelectedIndex == 2 && !Check_Overlay.Checked)
                {
                    foreach (DataGridViewRow row in dataGridView_savescan.Rows)
                        row.Cells["Select"].Value = false;
        ...
        {
            if (string.IsNullOrEmpty(Dir_Scan_For_New) &&
                string.IsNullOrEmpty(Dir_Scan_For_New) &&
                ScanFile_Formats == 0) return;
            ...
            add("ScanDir", Dir_Scan_For_New);
            add("DisplayDir", Dir_Scan_DataBase);
            add("FileFormats", ScanFile_Formats.ToString());
            add("CSVDelimiter", CSV_Delimiter == "\t" ? "TAB" : CSV_Delimiter);

            var file = Path.Combine(ConfigDir, "ScanPageSettings.xml");
        ...
        private Int32 SetDefaultConfig()
        {
            ...
            if ((ret = ScanConfig.SetConfigList()) == 0)
            {
                ...
                if (retIdx >= 0)
                { // If return a valid config index
        ...
        private ProgressBarForm PBW;

        // OPCIONAL -> manter eventos se ProgressBarForm estiver a "ouvir" isto; Convém tornar o invoke thread safe,
        // ou seja, garantir que o evento é chamado na thread do formulário principal, para evitar problemas de cross-thread operation.
        public static event Action RequestPBWClose = null;
        internal static bool SendPBWClose { set { RequestPBWClose?.Invoke(); } }
        ...
        internal static string SendPBWContentChange { set { RequestPBWContentChange?.Invoke(value); } }
        //internal static String SendPBWContentChange { set { RequestPBWContentChange(value); } }

        private void ProgressWindowStart(string title, string content, bool cancellable)
        {
            UI(() =>
            {
                SystemBusy(true);

                if (PBW != null && !PBW.IsDisposed)
                {
                    // Idealmente: expor método no PBW para atualizar label
                    // Caso uses eventos:
                    SendPBWContentChange = content;
                    PBW.TopMost = true;
                    PBW.BringToFront();
                    PBW.TopMost = false;
                    return;
                ...
                PBW = new ProgressBarForm(title, content, cancellable, formResize.form_ratio_width, formResize.form_ratio_height);
                PBW.StartPosition = FormStartPosition.CenterScreen;
                PBW.TopMost = true;

                // modeless para não bloquear; modal só se o trabalho pesado estiver fora do UI thread
                PBW.FormClosed += (_, __) => PBW = null;
                PBW.Show(this);
                PBW.BringToFront();
            });
        }
        ...
        //private void ProgressWindowStart(string title, string content, bool cancellable)
        //{
        //    // Garante que corre no UI thread
        //    if (InvokeRequired)
        //    {
        //        BeginInvoke((Action)(() => ProgressWindowStart(title, content, cancellable)));
        //        return;
        //    }
        ...
        //    if (PBW != null && !PBW.IsDisposed)
        //    {
        //        ProgressWindowContentUpdate(content);
        //        PBW.TopMost = true;
        //        PBW.BringToFront();
        //        PBW.TopMost = false;
        //        return;
        ...
        //    // Recomendado: modeless para não bloquear o UI thread
        //    PBW.Show(this);
        //    PBW.BringToFront();
        //}
        ...
        //    PBW.TopMost = true;
        //    //Application.Run(PBW);
        //    PBW.ShowDialog(this); //modal -> bloqueia janela principal até fechar

        //    //PBW.BringToFront(); //modeless -> não bloqueia
        //}
        ...
        private void ProgressWindowContentUpdate(string newContent)
        {
            if (InvokeRequired)
            {
                BeginInvoke((Action)(() => ProgressWindowContentUpdate(newContent)));
                return;
            }

            // Se o teu ProgressBarForm já tem um método/label, é melhor chamar diretamente.
            // Como já tens o event, mantemos:
            if (PBW != null && !PBW.IsDisposed)
                SendPBWContentChange = newContent;
        }
        ...
        //    if (InvokeRequired)
        //    {
        //        BeginInvoke((Action)(ProgressWindowCompleted));
        //        return;
        //    }
        ...
        #region API Database & Sample List 
        public Dictionary<string, object> BuildParameters()
        {
            var head = Scan.ScanConfigData.head;
            var section = Scan.ScanConfigData.section[0];

            return new Dictionary<string, object>()
            {
                ["Scan Config Name"] = head.config_name,
                ["Width"] = section.width_px,
                ["Exposure (ms)"] = section.exposure_time,
                ["Num. of Repeats"] = head.num_repeats,
                ["PGA Gain"] = Scan.PGA,
                ["System Temperature (ºC)"] = Scan.SensorData.Length > 0 ? Scan.SensorData[0] : (object)null,
        ...
        {
            public string username { get; set; } // Username of the person uploading the sample
            public string sample_name { get; set; } // Name of the sample
            public string material { get; set; } // Material of the sample
            public string subtype { get; set; }
            public string position { get; set; } // Position of the sample in the device
            public double[] absorbance { get; set; } // Absorbance data of the sample
            //public DateTime timestamp { get; set; }
            [JsonProperty("parameters")]
            public Dictionary<string, object> parameters { get; set; } // Additional parameters related to the sample
        }

        private List<string> predefmaterials = new List<string>
        {
            "Empty", "Extreme Caramel", "Extreme Vanilla", "Extreme Vanilla (American)",
            "Glue", "Gold", "Granulated Cork", "NIST", "Oil", "Olive Oil",
            "Reference", "Spicy", "Smokey"
        };

        /*
            DTO - Data Transfer Object
        
            Função utilitária para juntar os materiais pré-definidos + os vindos da API
        */
        private List<MaterialDto> GetMergedMaterials(List<MaterialDto> apiMaterials)
        {
            var merged = new List<MaterialDto>();
            foreach (var mat in predefmaterials)
            {
                if (!apiMaterials.Any(m => m.Name.Equals(mat, StringComparison.OrdinalIgnoreCase)))
                {
                    merged.Add(new MaterialDto { Name = mat, Id = 0 });
                }
            }
            merged.AddRange(apiMaterials);
            return merged;
        }
        ...
        public void SetupDataGridView()
        {
            ...
            //1) Position column (read-only)
            DataGridViewTextBoxColumn posCol = new DataGridViewTextBoxColumn();
            posCol.HeaderText = "Position";
            posCol.Name = "Position";
            posCol.ReadOnly = true; //filled when click on Save button of positions in Scanner
            dataGridView_Table.Columns.Add(posCol);

            //2) Sample Name column
            DataGridViewTextBoxColumn sampleCol = new DataGridViewTextBoxColumn();
            sampleCol.HeaderText = "Sample Name";
            sampleCol.Name = "SampleName";
            sampleCol.ReadOnly = false; //user can write here
            dataGridView_Table.Columns.Add(sampleCol);

            // 3) Subtype column 
            DataGridViewTextBoxColumn subtypeCol = new DataGridViewTextBoxColumn();
            subtypeCol.HeaderText = "Subtype";
            subtypeCol.Name = "Subtype";
            subtypeCol.ReadOnly = false; // permite edição do user 
            dataGridView_Table.Columns.Add(subtypeCol);

            //4) Material column
            DataGridViewComboBoxColumn materialCol = new DataGridViewComboBoxColumn();
            materialCol.HeaderText = "Material";
            materialCol.Name = "Material";
            if (materialCol != null)
            {
                materialCol.DataSource = materials;
                materialCol.DisplayMember = "Name";
                materialCol.ValueMember = "Name";
            }
            //materialCol.Items.AddRange(predefmaterials.ToArray());
            //materialCol.Items.AddRange("Empty","Extreme Caramel","Extreme Vanilla", "Extreme Vanilla (American)" ,"Glue", "Gold", "Granulated Cork", "NIST","Oil","Olive Oil", "Reference", "Spicy", "Smokey");
            dataGridView_Table.Columns.Add(materialCol);

            //5) Type of Measurement column
            DataGridViewTextBoxColumn typeCol = new DataGridViewTextBoxColumn();
            typeCol.HeaderText = "Type of Measurement";
            typeCol.Name = "TypeOfMeasurement";
            typeCol.ReadOnly = true; //filled automatically
            dataGridView_Table.Columns.Add(typeCol);

            //6) Mode of Measurement column
            DataGridViewTextBoxColumn modeCol = new DataGridViewTextBoxColumn();
            modeCol.HeaderText = "Mode of Measurement";
            modeCol.Name = "ModeOfMeasurement";
            modeCol.ReadOnly = true; //filled when user sets/gets Sensor Config done
            dataGridView_Table.Columns.Add(modeCol);
        }

        private async void button_AddMaterial_Click(object sender, EventArgs e)
        {
            string newMaterial = textBox_NewMaterial.Text.Trim();
            if (!string.IsNullOrEmpty(newMaterial))
            {
                // Capitalizar o nome, se quiseres
                newMaterial = CultureInfo.CurrentCulture.TextInfo.ToTitleCase(newMaterial.ToLower());

                bool added = await AddNewMaterialAsync(newMaterial);

                if (added) textBox_NewMaterial.Clear();
            }
        }

        private async Task<bool> AddNewMaterialAsync(string materialName)
        {
            try
            {
                var payload = new { name = materialName };
                var response = await _apiClient.PostAsJsonAsync("api/materials/", payload);
                if (!response.IsSuccessStatusCode)
                {
                    MessageBox.Show("Failed to add material. It may already exist.");
                    return false;
                }
                await InitializeDataAsync(); // <--- Refreshes your ComboBox_Material
                return true;
            }
            catch (Exception ex)
            {
                MessageBox.Show("Erro na atualização dos materiais: " + ex.Message);
                return false;
            }
        }
        ...
        private void dataGridView_Table_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (dataGridView_Table.CurrentCell == null) return;

            var cell = dataGridView_Table.CurrentCell;
            int row = cell.RowIndex;
            int col = cell.ColumnIndex;
            string colName = dataGridView_Table.Columns[col].Name;

            //
            // 1) Ctrl+I on “SampleName” column ⇒ fill downward with incrementing names
            //
            if (e.Control && e.KeyCode == Keys.I && colName == "SampleName")
            {
                ...
                // Try to split into (prefix)(digits) at the end
                // e.g. "azeite1" ⇒ prefix="azeite", startNum=1
                ...
                if (m.Success)
                {
                    ...
                    nextNum = int.Parse(m.Groups[2].Value);
                }
                else
                {
                    // no trailing digits, treat entire text as prefix and start from 1
                    prefix = text;
                    nextNum = 1;
                }

                // Fill this row and every row below
                for (int r = row; r < dataGridView_Table.Rows.Count; r++)
                {
                    if (dataGridView_Table.Rows[r].IsNewRow)
                        break;

                    dataGridView_Table[col, r].Value = prefix + nextNum;
                    nextNum++;
                }
            ...
            //
            // 2) Ctrl+F on “Material” column ⇒ fill downward with the same material
            //
            else if (e.Control && e.KeyCode == Keys.F && colName == "Material")
            {
                var text = cell.Value as string;
                if (string.IsNullOrWhiteSpace(text))
                    return;
                ...
                for (int r = row; r < dataGridView_Table.Rows.Count; r++)
                {
                    ...
                    dataGridView_Table[col, r].Value = text;
                }

                e.Handled = true;
            }
            else if (e.Control && e.KeyCode == Keys.S && colName == "Subtype")
            {
                // 1. Find the start row for sequence (skip reference row)
                int startRow = row;
                // Optional: adjust for ref (e.g. first row is "ref")
                // You can tweak this logic if your ref row is always index 0.
                if (row == 0) return;

                // 2. Collect the sequence from filled rows above, up to this row
                List<string> subtypeSequence = new List<string>();
                int r = startRow;
                while (r >= 0)
                {
                    var val = dataGridView_Table[col, r].Value as string;
                    if (string.IsNullOrWhiteSpace(val)) break; // stop at first empty
                    subtypeSequence.Insert(0, val); // insert at beginning
                    r--;
                    if (r == 0) break; // stop at "ref" if needed
                }
                if (subtypeSequence.Count == 0) return;

                // 3. Fill downward with the collected sequence, cycling as needed
                int seqIndex = 0;
                for (int rr = startRow + 1; rr < dataGridView_Table.Rows.Count; rr++)
                {
                    if (dataGridView_Table.Rows[rr].IsNewRow)
                        break;
                    dataGridView_Table[col, rr].Value = subtypeSequence[seqIndex % subtypeSequence.Count];
                    seqIndex++;
                }
        ...
        //criam-se campos de instâncias para ser de mais fácil acesso a partir de outros métodos
        public List<string> samplePositions = new List<string>();

        #region Helpers CheckBox_Pos
        public void FindCheckBoxes(Control parent)
        {
            foreach (Control c in parent.Controls)
            {
                if (c is CheckBox checkBox && checkBox.Checked && checkBox.Name.StartsWith("checkBox_Pos"))
                {
                    samplePositions.Add(checkBox.AccessibleName);
                }

                //Se o controlo for um container como GroupBox ou Panel, inicia a busca por checkboxes dentro destes também
                if (c.HasChildren)
                {
                    FindCheckBoxes(c);
                }
        ...
        public void ClearCheckBoxes(Control parent)
        {
            foreach (Control c in parent.Controls)
            {
                if (c is CheckBox checkBox && checkBox.Checked && checkBox.Name.StartsWith("checkBox_Pos"))
        ...
        #region Config. Portas
        public async Task<bool> ConfigurarPorta(string port)
        {
            try
            {
                // Fecha porta aberta, se existir
                if (serialPort != null && serialPort.IsOpen)
                    serialPort.Close();

                serialPort = new SerialPort(port, 115200)
                {
                    ReadTimeout = 500,       // definimos um timeout razoável
                    NewLine = "\n",          // para ReadLine()
                    DtrEnable = true,        // conforme precisas
                    RtsEnable = true
                };
                serialPort.Open();

                await Task.Delay(5000); // Dar tempo ao hardware para inicializar — sem bloquear o thread!
                return true;
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error on configuration of COM port:\n" + ex.Message,
                                "ConfigurarPorta", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }
        #endregion

        #region Wrapper para Scan
        private TaskCompletionSource<RunWorkerCompletedEventArgs> _scanTcs;
        private Task<RunWorkerCompletedEventArgs> StartScanAsync()
        {
            _scanTcs = new TaskCompletionSource<RunWorkerCompletedEventArgs>();
            bwScan.RunWorkerAsync();                // dispara o DoScan
            return _scanTcs.Task;                   // aguarda o Completed
        }
        #endregion
        ...
        private Dictionary<string, (double X, double Y, double Z)> coordenadasPosicoes = new Dictionary<string, (double, double, double)>
        {
            { "Pos1", (199.0, -70.0, 8.0) },
            { "Pos2", (159.0, -70.0, 8.0) },
            { "Pos3", (199.0, -110.0, 8.0) },
            { "Pos4", (159.0, -110.0, 8.0) },
            { "Pos5", (199.0, -150.0, 8.0) },
            { "Pos6", (159.0, -150.0, 8.0) },
            { "Pos7", (89.0, -70.0, 8.0) },
            { "Pos8", (49.0, -70.0, 8.0) },
            { "Pos9", (89.0, -110.0, 8.0) },
            { "Pos10", (49.0, -110.0, 8.0) },
            { "Pos11", (89.0, -150.0, 8.0) },
            { "Pos12", (49.0, -150.0, 8.0) },
            { "Pos13", (199.0, -205.0, 8.0) },
            { "Pos14", (159.0, -205.0, 8.0) },
            { "Pos15", (199.0, -245.0, 8.0) },
            { "Pos16", (159.0, -245.0, 8.0) },
            { "Pos17", (199.0, -285.0, 8.0) },
            { "Pos18", (159.0, -285.0, 8.0) },
            { "Pos19", (84.0, -205.0, 8.0) },
            { "Pos20", (44.0, -205.0, 8.0) },
            { "Pos21", (84.0, -245.0, 8.0) },
            { "Pos22", (44.0, -245.0, 8.0) },
            { "Pos23", (84.0, -285.0, 8.0) },
            { "Pos24", (44.0, -285.0, 8.0) }
        };

        /*private Dictionary<string, (double X, double Y, double Z)> coordenadasPosicoes = new Dictionary<string, (double, double, double)>
        {
            { "Pos1", (199.0, -70.0, 10.0) },
            { "Pos2", (159.0, -70.0, 10.0) },
            { "Pos3", (199.0, -110.0, 10.0) },
            { "Pos4", (159.0, -110.0, 10.0) },
            { "Pos5", (199.0, -150.0, 10.0)},// 10.0) },
            { "Pos6", (159.0, -150.0, 10.0)},//10.0) },
            { "Pos7", (89.0, -70.0, 10.0) },
            { "Pos8", (49.0, -70.0, 10.0) },
            { "Pos9", (89.0, -110.0, 10.0) },
            { "Pos10", (49.0, -110.0, 10.0) },
            { "Pos11", (89.0, -150.0, 10.0) },
            { "Pos12", (49.0, -150.0, 10.0) },
            { "Pos13", (206.0, -198.0, 10.0) },
            { "Pos14", (186.0, -198.0, 10.0) },
            { "Pos15", (166.0, -198.0, 10.0) },
            { "Pos16", (149.0, -198.0, 10.0) },
            { "Pos17", (206.0, -217.0, 10.0) },
            { "Pos18", (186.0, -217.0, 10.0) },
            { "Pos19", (166.0, -217.0, 10.0) },
            { "Pos20", (149.0, -217.0, 10.0) },
            { "Pos21", (206.0, -236.0, 10.0) },
            { "Pos22", (186.0, -236.0, 10.0) },
            { "Pos23", (166.0, -236.0, 10.0) },
            { "Pos24", (149.0, -236.0, 10.0) },
            { "Pos25", (206.0, -255.0, 10.0) },
            { "Pos26", (186.0, -255.0, 10.0) },
            { "Pos27", (166.0, -255.0, 10.0) },
            { "Pos28", (149.0, -255.0, 10.0) },
            { "Pos29", (206.0, -274.0, 10.0) },
            { "Pos30", (186.0, -274.0, 10.0) },
            { "Pos31", (166.0, -274.0, 10.0) },
            { "Pos32", (149.0, -274.0, 10.0) },
            { "Pos33", (206.0, -293.0, 10.0) },
            { "Pos34", (186.0, -293.0, 10.0) },
            { "Pos35", (166.0, -293.0, 10.0) },
            { "Pos36", (149.0, -293.0, 10.0) },
            { "Pos37", (84.0, -205.0, 10.0) },
            { "Pos38", (44.0, -205.0, 10.0) },
            { "Pos39", (84.0, -255.0, 10.0) },
            { "Pos40", (44.0, -255.0, 10.0) },
            { "Pos41", (84.0, -285.0, 10.0) },
            { "Pos42", (44.0, -285.0, 10.0) }
        };*/

        private SerialPort serialPort;
        private string selectedCOMprinter = string.Empty;
        public class ComboItem
        {
            public string Text { get; set; } //O que vai aparecer no dropdown
            public string Value { get; set; } //O valor real associado ao item (Comx)
            ...
            try
            {
                using (var searcher = new ManagementObjectSearcher("SELECT * From Win32_PnPEntity WHERE Name LIKE '%(COM%'"))
                {
                    foreach (var device in searcher.Get())
                    {
                        ...
                        //extração do nome completo para apenas a indicação da porta
                        int startIndex = name.LastIndexOf("(COM");
                        if (startIndex != -1)
                        {
                            string portName = name.Substring(startIndex + 1).Replace("(", "").Replace(")", "");
                ...
                {
                    comboBox_COMport.Items.Add("No Ports Available");
                    comboBox_COMport.SelectedIndex = 0;
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error on listing available COM ports: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }*/
        ...
        public void ListCOMports()
        {
            ...
            try
            {
                List<(int comNumber, string displayName)> ports = new List<(int, string)>();
                ...
                using (var searcher = new ManagementObjectSearcher("SELECT * From Win32_PnPEntity WHERE Name LIKE '%(COM%'"))
                {
                    foreach (var device in searcher.Get())
                    {
                        ...
                        string portName = name.Substring(startIndex + 1)
                                .Replace("(", "")
                                .Replace(")", "");

                        // Extrai número da porta (ex.: COM15 → 15)
                        int comNumber = int.Parse(
                            new string(portName.Where(char.IsDigit).ToArray())
                        );

                        // Identifica o tipo do dispositivo
                        string type = IdentifyPortType(name, pnpId);

                        string display = $"{portName} - {type}";

                        ports.Add((comNumber, display));

                    }   
                }

                // Ordenar por número da COM (ex.: COM3, COM4, COM15…)
                foreach (var port in ports.OrderBy(p => p.comNumber))
                {
                    comboBox_COMport.Items.Add(new ComboItem
                    {
                        Text = port.displayName,               // Ex.: "COM15 - Printer"
                        Value = $"COM{port.comNumber}"         // Ex.: "COM15" → usado para abrir a porta
                    });
                }
        ...
        private string IdentifyPortType(string name, string pnpId)
        {
            string id = (name + " " + pnpId).ToUpper();

            if (id.Contains("CH340") || id.Contains("CH341"))
                return "3 Axis System";

            if (id.Contains("FTDI") || id.Contains("FT232") || id.Contains("FT245"))
                return "FTDI Adapter";

            if (id.Contains("USB\\VID_10C4"))  // Silicon Labs
                return "SiliconLabs CP210x";

            if (id.Contains("CDC") || id.Contains("ACM"))
                return "CDC Serial Device";

            //// Se for uma impressora específica tua (mapeamento manual)
            //if (id.Contains("VID_1A86") && id.Contains("PID_7523"))
            //    return "Impressora";  // CH340 do tipo que usas

            // Caso não detecte nada específico
            return "Serial Device";
        }

        private async Task<bool> MovePrinterToPosition(string posName)
        {
            if (!coordenadasPosicoes.TryGetValue(posName, out var coords))
            {
                MessageBox.Show($"Coordinates not found for {posName}",
                                "MovePrinter", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        ...
        private async void button_Save_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please select a valid PORT COM", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            //selectedCOMprinter = comboBox_COMport.SelectedItem.ToString();
            ComboItem item = comboBox_COMport.SelectedItem as ComboItem;
            ...
            selectedCOMprinter = item.Value;  // <-- "COM15", não "COM15 - Printer"
            // 2) aguarda a configuração da porta
            bool opened = await ConfigurarPorta(selectedCOMprinter);
            if (!opened)
            ...
            {
                MessageBox.Show("Please select at least one position.", "Error",
                                MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            groupBox_plate1.Enabled = false;
            groupBox_plate2.Enabled = false;
            groupBox_plate3.Enabled = false;
            groupBox_plate4.Enabled = false;
            ...
            for (int i = 0; i < samplePositions.Count; i++)
            {
                ...
                var type = i == 0 ? "Reference" : "Sample";
                var mode = label_ActiveConfig.Text.StartsWith("Column") ? "Col" : "Had";

                // adiciona uma linha, preenchendo cada coluna
                int idx = dataGridView_Table.Rows.Add();
                var row = dataGridView_Table.Rows[idx];
                row.Cells["Position"].Value = pos;
                row.Cells["SampleName"].Value = "";      // user preenche depois
                row.Cells["Material"].Value = "";      // idem
                row.Cells["TypeOfMeasurement"].Value = type;
                row.Cells["ModeOfMeasurement"].Value = mode;
            }
        }

        private void button_Undo_Click(object sender, EventArgs e)
        {
            // 1) limpa as checkboxes
            ClearCheckBoxes(this.groupBox_plate1);
            ClearCheckBoxes(this.groupBox_plate2);
            ClearCheckBoxes(this.groupBox_plate3);
            ClearCheckBoxes(this.groupBox_plate4);

            // 2) limpa a lista de amostras
            dataGridView_Table.Rows.Clear();

            // 3) restaura UI
            groupBox_plate1.Enabled = true;
            groupBox_plate2.Enabled = true;
            groupBox_plate3.Enabled = true;
            groupBox_plate4.Enabled = true;
            button_Save.Enabled = true;
            button_Undo.Enabled = false;
        }

        private void OnTestSequenceComplete()
        {
            button_Save.Enabled = false;
            groupBox_plate1.Enabled = false;
            groupBox_plate2.Enabled = false;
        ...
        private bool ValidateSampleList()
        {
            ...
            {
                // skip the blank “new row” at the bottom
                if (row.IsNewRow) continue;

                var sampleName = row.Cells["SampleName"].Value as string;
                var material = row.Cells["Material"].Value as string;

                if (string.IsNullOrWhiteSpace(sampleName))
                {
                    MessageBox.Show(
                        "Please fill in the 'Sample Name' for every row in the table.",
                        "Missing Sample Name",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Warning);
                    row.Cells["SampleName"].Selected = true;
                    return false;
                }
                ...
                {
                    MessageBox.Show(
                        "Please select a 'Material' for every row in the table.",
                        "Missing Material",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Warning);
                    row.Cells["Material"].Selected = true;
                    return false;
                }
            }
            return true;
        ...
        private Image _heatmapOriginal;
        private float _zoom = 1.0f;              // 1 = 100%
        private const float ZoomStep = 0.1f;     // 10% per wheel notch
        private const float ZoomMin = 0.2f;      // 20%
        private const float ZoomMax = 5.0f;      // 500%
        ...
        private async Task LoadHeatmapAsync()
        {
            try
            {
                ...
                using (var imgStream = await response.Content.ReadAsStreamAsync())
                {
                    ...
                    _heatmapOriginal?.Dispose();
                    _heatmapOriginal = new Bitmap(img);
        ...
        // === wheel handler ===
        private void pictureBox_heatMap_MouseWheel(object sender, MouseEventArgs e)
        {
            if (_heatmapOriginal == null) return;

            _zoom = e.Delta > 0 ? Math.Min(_zoom + ZoomStep, ZoomMax)
        ...
        private void ApplyZoom()
        {
            ...
            // scale a fresh bitmap from the original so quality doesn’t degrade
            int w = Math.Max(1, (int)(_heatmapOriginal.Width * _zoom));
            int h = Math.Max(1, (int)(_heatmapOriginal.Height * _zoom));
            ...
            using (var g = Graphics.FromImage(bmp))
            {
                ...
                g.DrawImage(_heatmapOriginal, new Rectangle(0, 0, w, h));
            }

            // swap in the new bitmap (dispose the old one to avoid leaks)
            var old = pictureBox_heatMap.Image;
            pictureBox_heatMap.Image = bmp;
            old?.Dispose();
        }
        #endregion
        ...
        private async void button_TestSequence_Click(object sender, EventArgs e)
        {
            ...
            // 0) valida pasta
            if (string.IsNullOrWhiteSpace(Dir_Scan_For_New) || !Directory.Exists(Dir_Scan_For_New))
            {
                MessageBox.Show("Please select first a Directory path.",
                  "Undefined path to save .csv files",
                  MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            for (int i = 0; i < samplePositions.Count; i++)
            {
                if (firstScan == true)
                {
                    serialPort.WriteLine("G1 Z8.0 F1500");
                }

                var pos = samplePositions[i];
                var row = dataGridView_Table.Rows[i];
                // Lê os valores que o utilizador pôs na tabela
                var position = row.Cells["Position"].Value as string;
                var type = row.Cells["TypeOfMeasurement"].Value as string;
                var sampleName = row.Cells["SampleName"].Value as string;
                var material = row.Cells["Material"].Value as string;
                ...
                if (!await MovePrinterToPosition(pos))
                {
                    MessageBox.Show($"Failed to move to {pos}", "Error",
                                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                    allSuccess = false;
                    break;
                }

                // Executa o scan (novo na 1ª, previous nas restantes)
                var refType = firstScan
                    ? Scan.SCAN_REF_TYPE.SCAN_REF_NEW
                    : Scan.SCAN_REF_TYPE.SCAN_REF_PREV;

                this.Text = $"{(firstScan ? "Reference" : "Sample")} scan at {pos}";
                int result = await Task.Run(() => Scan.PerformScan(refType));
                if (result != SDK.RETURN_PASS)
                {
                    MessageBox.Show($"Scan failed at {pos} (code {result})", "Error",
                                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                    allSuccess = false;
                    break;
                }
                ...
                var toPlot = RadioButton_Reference.Checked ? ChartData_RefIntensity
                            ...
                            : RadioButton_Absorbance.Checked ? ChartData_Absorbance
                ...
                // 4) Se for a última posição e tudo correu bem, envia o parking move
                if (i == samplePositions.Count - 1 && allSuccess)
                {
                    serialPort.WriteLine("G1 X-26.0 Y-6.0 Z8.0 F2000");
                    serialPort.WriteLine("G1 Z0.0 F1500");
                    this.Text = "Parking move sent";
                    await Task.Delay(2000);
                }
            }

            OnTestSequenceComplete();
            ...
            try
            {
                ...
                var uploadResponse = await _apiClient.PostAsync("api/upload-sample/", uploadContent);
                uploadResponse.EnsureSuccessStatusCode();

                // Lê o corpo da resposta e guarda o último sequence_id
                var json = await uploadResponse.Content.ReadAsStringAsync();
                dynamic respObj = JsonConvert.DeserializeObject(json);

                // Guarda o sequence_id do último batch
                latestSequenceId = (int)respObj.sequence_id;
            }
            catch (HttpRequestException httpEx)
            {
                MessageBox.Show($"Batch upload failed:\n{httpEx.Message}",
            ...
            {
                MessageBox.Show($"Error fetching heatmap:\n{httpEx.Message}",
                    "API Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                this.Text = "Sequence complete (no image)";
            }
            finally
            {
                button_TestSequence.Enabled = true;
        ...
        private GLineSeries BuildGLineForCurrentScan(Scan.SCAN_REF_TYPE refType, string posName)
        {
            // 1) Prepara os valores X (wavelength) e Y (intensity ou reference)
            var wavelengths = Scan.WaveLength
                                .Select(d => Math.Round(d, 2, MidpointRounding.AwayFromZero))
                                .ToArray();

            // escolhe o array de Y conforme o tipo de scan
            IList<double> yData = (refType == Scan.SCAN_REF_TYPE.SCAN_REF_NEW)
                ? Scan.ReferenceIntensity.Select(v => (double)v).ToList()
                : Scan.Intensity.Select(v => (double)v).ToList();

            // 2) monta o título da série
            // primeiro scan de referência usa "(Ref)", os seguintes usam só a posição
            string title = refType == Scan.SCAN_REF_TYPE.SCAN_REF_NEW
                ? $"{posName} (Ref)"
                : posName;

            // 3) popula os pontos
            var chartValues = new GearedValues<CustomerVm>();
            for (int i = 0; i < Math.Min(wavelengths.Length, yData.Count); i++)
            {
                chartValues.Add(new CustomerVm
                {
                    ...
                    fileName = title,
                    serialNumber = Device.DevInfo.SerialNumber,
                    temp = Scan.SensorData.Length > 0 ? Scan.SensorData[0].ToString() : "",
                    humi = Scan.SensorData.Length > 2 ? Scan.SensorData[2].ToString() : ""
                });
            }

            // 4) constrói e retorna o GLineSeries
            return new GLineSeries
            {
                Values = chartValues,
                Title = title,
                Stroke = StrokeColors[
                                    // garante que a mesma cor de overlay se repete
                                    (refType == Scan.SCAN_REF_TYPE.SCAN_REF_NEW
                                        ? ChartData_RefIntensity.Count
                                        : ChartData_Intensity.Count
                                    ) % NumOfStrokeColors
                                ],
                StrokeThickness = 1,
                Fill = System.Windows.Media.Brushes.Transparent,
        ...
        private void AddSeriesForCurrentScan(Scan.SCAN_REF_TYPE refType, string posName)
        {
            // 2.1) referência (só no primeiro scan)
            if (refType == Scan.SCAN_REF_TYPE.SCAN_REF_NEW)
            {
                var sRef = BuildGLineForCurrentScan(Scan.SCAN_REF_TYPE.SCAN_REF_NEW, posName);
                ChartData_RefIntensity.Add(sRef);
            }

            // 2.2) intensidade
            var sInt = BuildGLineForCurrentScan(Scan.SCAN_REF_TYPE.SCAN_REF_PREV, posName);
            ChartData_Intensity.Add(sInt);

            // 2.3) absorbância (se tiver sido calculada)
            if (Scan.Absorbance != null && Scan.Absorbance.Count == Scan.ScanDataLen)
            {
                var valuesA = new GearedValues<CustomerVm>();
                var xs = Scan.WaveLength.Select(d => Math.Round(d, 2)).ToArray();
                for (int i = 0; i < xs.Length; i++)
                {
                    valuesA.Add(new CustomerVm
                    {
                        x = xs[i],
                        y = Scan.Absorbance[i],
                        fileName = $"{posName} (Abs)",
                        serialNumber = Device.DevInfo.SerialNumber,
                        temp = Scan.SensorData.Length > 0 ? Scan.SensorData[0].ToString() : "",
                        humi = Scan.SensorData.Length > 2 ? Scan.SensorData[2].ToString() : ""
                    });
            ...
            // 2.4) reflectância (se tiver sido calculada)
            if (Scan.Reflectance != null && Scan.Reflectance.Count == Scan.ScanDataLen)
            {
                var valuesR = new GearedValues<CustomerVm>();
                var xs = Scan.WaveLength.Select(d => Math.Round(d, 2)).ToArray();
                for (int i = 0; i < xs.Length; i++)
                {
                    valuesR.Add(new CustomerVm
                    {
                        x = xs[i],
                        y = Scan.Reflectance[i],
                        fileName = $"{posName} (Refl)",
                        serialNumber = Device.DevInfo.SerialNumber,
                        temp = Scan.SensorData.Length > 0 ? Scan.SensorData[0].ToString() : "",
                        humi = Scan.SensorData.Length > 2 ? Scan.SensorData[2].ToString() : ""
                    });
        ...
        private void ShowPlot(List<GLineSeries> allSeries)
        {
            // overlay = true → mostra tudo; overlay = false → só última série
            bool overlay = Check_Overlay.Checked;
            if (allSeries == null || allSeries.Count == 0)
                return;

            // escolhe quais exibir
            IEnumerable<GLineSeries> toDisplay = overlay
                ? (IEnumerable<GLineSeries>)allSeries
                : new GLineSeries[] { allSeries.Last() };

            RefreshPlot();

            MyChart.Series.AddRange(toDisplay);
        }
        ...
        private async void button_AddSubtype_Click(object sender, EventArgs e)
        {
            var mat = SelectedMaterial;
            var selectedSubtype = comboBox_Subtype.SelectedItem as SubMaterialDto;

            if (mat == null || selectedSubtype == null)
            {
                MessageBox.Show("Seleciona um material e um subtipo primeiro!");
                return;
            }

            // Verifica se já foi adicionado
            if (listBox_SelectedSubtypes.Items.Cast<SubMaterialDto>().Any(x => x.Id == selectedSubtype.Id))
            {
                MessageBox.Show("Esse subtipo já está selecionado!");
                return;
            }

            // 1) Chama a API para contar amostras deste material/subtipo
            int sampleCount = await GetSampleCountAsync(mat.Id, selectedSubtype.Id);

            if (sampleCount < 20)
            {
                MessageBox.Show(
                    $"O subtipo '{selectedSubtype.Name}' tem apenas {sampleCount} amostras.\nPor favor seleciona um subtipo com pelo menos 20 amostras.",
                    "Poucas amostras",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning
                );
                return;
            ...
            // 2) Adiciona à lista (apenas se passou o teste!)
            listBox_SelectedSubtypes.Items.Add(selectedSubtype);
            await LoadAnnotationKeysAsync();
        }

        // Exemplo de função para pedir à API o nº de amostras (ajusta o endpoint conforme necessário)
        private async Task<int> GetSampleCountAsync(int materialId, int subtypeId)
        {
            // Exemplo de endpoint: /api/samples/count?material=XX&subtype=YY
            var response = await _apiClient.GetAsync($"api/samples/count?material={materialId}&subtype={subtypeId}");
            if (!response.IsSuccessStatusCode)
                return 0;

            var content = await response.Content.ReadAsStringAsync();
            // Supondo que devolve {"count": 23}
            dynamic json = JsonConvert.DeserializeObject(content);
            return (int)json.count;
        }

        private async void btn_ClearSubtypes_Click(object sender, EventArgs e)
        {
            listBox_SelectedSubtypes.Items.Clear();
            await LoadAnnotationKeysAsync();
        }
        ...
        private async void btnTrain_Click(object sender, EventArgs e)
        {
            ...
            btn_ClearSubtypes.Enabled = false;
            button_AddSubtype.Enabled = false;
            ProgressWindowStart("Training", "Training model, please wait...", false);
            try
            {
                // get material name & subtype from the ComboBox
                var mat = SelectedMaterial;
                var chosenSubtypes = listBox_SelectedSubtypes.Items.Cast<SubMaterialDto>().ToList();
                string subtypesInfo = string.Join(" vs ", chosenSubtypes.Select(s => s.Name));
                ...
                if (chosenSubtypes.Count < 2)
                {
                    MessageBox.Show("Select at least 2 subtypes with more than 20 samples each");
                    return;
                }

                string annotationKey = comboBox_annKey?.SelectedItem?.ToString()?.Trim();

                if (string.IsNullOrEmpty(annotationKey) && chosenSubtypes.Count < 2)
                {
                    MessageBox.Show("Select at least 2 subtypes OR enter an annotation key.");
                    return;
                }
                ...
                var payload = new TrainRequest
                {
                    ...
                    Subtypes = chosenSubtypes.Select(s => s.Id).ToList(), 
                ...
                textBox_ModelInfo.Text =
                    ...
                    (string.IsNullOrEmpty(annotationKey)
                        ? $"Subtypes: {subtypesInfo}\r\n"
            ...
            finally
            {
                ...
                btn_ClearSubtypes.Enabled = true;
                button_AddSubtype.Enabled = true;
                listBox_SelectedSubtypes.Items.Clear();
                ProgressWindowCompleted();
            }
        }
        ...
        private async Task<List<SubMaterialDto>> GetSubMaterialsAsync(int materialId)
        {
            var response = await _apiClient.GetAsync($"api/submaterials/?material_id={materialId}");
            if (!response.IsSuccessStatusCode)
            {
                MessageBox.Show("Erro ao buscar subtipos!");
                return new List<SubMaterialDto>();
            }
            var content = await response.Content.ReadAsStringAsync();
            var subMaterials = JsonConvert.DeserializeObject<List<SubMaterialDto>>(content);
            return subMaterials;
        }
        ...
        private async Task InitializeDataAsync()
        {
            try
            {
                var apimaterials = await _apiClient.GetFromJsonAsync<List<MaterialDto>>("api/materials/");
                materials = GetMergedMaterials(apimaterials);

                comboBox_Material.DataSource = null;
                comboBox_Material.DisplayMember = "Name";
                comboBox_Material.ValueMember = "Id"; //antes "Name"
                comboBox_Material.DataSource = materials;

                //comboBox_ModelMaterial.DataSource = null;
                //comboBox_ModelMaterial.DisplayMember = "Name";
                //comboBox_ModelMaterial.ValueMember = "Id"; //antes "Name"
                //comboBox_ModelMaterial.DataSource = materials;

                // DataGridViewColumn - refresh depois de adicionar colunas!
                var materialCol = dataGridView_Table.Columns["Material"] as DataGridViewComboBoxColumn;
                if (materialCol != null)
                {
                    materialCol.DataSource = null;
                    materialCol.DataSource = materials;
                    materialCol.DisplayMember = "Name";
                    materialCol.ValueMember = "Name";
                }
        ...
        private async Task LoadTrainedModelsAsync()
        {
            try
            {
                var response = await _apiClient.GetAsync("modeling/models/");
                if (!response.IsSuccessStatusCode)
                {
                    MessageBox.Show("Erro ao obter modelos treinados da API.");
                    return;
                }

                var json = await response.Content.ReadAsStringAsync();
                var models = JsonConvert.DeserializeObject<List<TrainedModelDto>>(json);

                if (models == null || models.Count == 0)
                {
                    comboBox_ModelMaterial.DataSource = null;
                    comboBox_ModelMaterial.Items.Clear();
                    comboBox_ModelMaterial.Text = "Nenhum modelo treinado";
                    return;
                }

                // Exibir nome formatado: "Material – Modelo (Data)"
                foreach (var m in models)
                {
                    m.Name = $"{m.Name}";
                }

                comboBox_ModelMaterial.DataSource = models;
                comboBox_ModelMaterial.DisplayMember = "Name";
                comboBox_ModelMaterial.ValueMember = "Id";
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Erro ao carregar modelos treinados:\n{ex.Message}");
            }
        }

        private async Task LoadAnnotationKeysAsync()
        {
            try
            {
                var mat = SelectedMaterial;
                if (mat == null)
                {
                    comboBox_annKey.DataSource = null;
                    comboBox_annKey.Items.Clear();
                    comboBox_annKey.Text = "";
                    return;
                }

                // Subtypes selecionados (caso existam)
                var selectedSubtypes = listBox_SelectedSubtypes.Items.Cast<SubMaterialDto>().ToList();
                var subtypeIds = selectedSubtypes.Select(s => s.Id).ToList();

                // Montar querystring
                var query = $"material_id={mat.Id}";
                if (subtypeIds.Any())
                {
                    var ids = string.Join("&subtype_ids[]=", subtypeIds);
                    query += $"&subtype_ids[]={ids}";
                }

                var response = await _apiClient.GetAsync($"editor/annotation-keys/?{query}");
                if (!response.IsSuccessStatusCode)
                {
                    comboBox_annKey.DataSource = null;
                    comboBox_annKey.Items.Clear();
                    comboBox_annKey.Text = "No keys";
                    return;
                }

                var json = await response.Content.ReadAsStringAsync();
                dynamic data = JsonConvert.DeserializeObject(json);
                var keys = ((JArray)data.keys).ToObject<List<string>>();

                comboBox_annKey.DataSource = keys;
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Erro ao carregar annotation keys:\n{ex.Message}");
            }
        }


        private async void comboBox_Material_SelectedIndexChanged(object sender, EventArgs e)
        {
            var selectedMaterial = comboBox_Material.SelectedItem as MaterialDto;
            if (selectedMaterial == null)
            {
                comboBox_Subtype.DataSource = null;
                return;
            }
            var subMaterials = await GetSubMaterialsAsync(selectedMaterial.Id);
            comboBox_Subtype.DataSource = null;
            comboBox_Subtype.DisplayMember = "Name";
            comboBox_Subtype.ValueMember = "Id";
            comboBox_Subtype.DataSource = subMaterials;
        ...
        */
        public class PredictionItem
        {
            ...
            public string predicted_class { get; set; } //para classification models
            public Dictionary<string, double> probabilities { get; set; } //para classification models
            public double? predicted_value { get; set; } //para regression models
        }

        //private TrainedModelDto SelectedModelMaterial => comboBox_Model.SelectedItem as TrainedModelDto;

        // Helper » Extracts numeric part from position string for sorting
        private static int PosKey(string pos)
        {
            if (string.IsNullOrEmpty(pos)) return int.MaxValue;
            var m = System.Text.RegularExpressions.Regex.Match(pos, @"\d+");
            return m.Success ? int.Parse(m.Value) : int.MaxValue;
        }
        ...
        private async void btnPredict_Click(object sender, EventArgs e)
        {
            ...
            var selectedModel = comboBox_ModelMaterial.SelectedItem as TrainedModelDto;
            if (selectedModel == null)
            {
                MessageBox.Show("Please select a trained model first.",
                                "Missing Model",
                                MessageBoxButtons.OK,
                                MessageBoxIcon.Warning);
                return;
            ...
            //int materialId = SelectedModelMaterial.MaterialId; 

            //if (materialId <= 0)
            //{
            //    MessageBox.Show("This model is not linked to a material.",
            //                    "Invalid Model",
            //                    MessageBoxButtons.OK,
            //                    MessageBoxIcon.Warning);
            //    return;
            ...
            //// Get selected material and submaterial
            ...
            try
            {
                ...
                // Se não houver predições, aborta
                if (result?.predictions == null || result.predictions.Count == 0)
                {
                    MessageBox.Show("No predictions returned.", "Empty Result", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                ...
                // Detectar se é regression ou classification
                bool isRegression = result.predictions.Any(p => p.predicted_value.HasValue);

                if (isRegression)
                {
                    dt.Columns.Add("Predicted Value");

                    foreach (var pred in result.predictions.OrderBy(p => PosKey(p.position)))
                    {
                        var row = dt.NewRow();
                        row["Position"] = pred.position;
                        row["Predicted Value"] = pred.predicted_value?.ToString("F2"); // 2 casas decimais
                        dt.Rows.Add(row);
                    }
                }
                else
                {
                    ...
                    // Descobrir todas as classes
                    var allClassNames = new HashSet<string>();
                    foreach (var pred in result.predictions.OrderBy(p => PosKey(p.position)))
                        if (pred.probabilities != null)
                    ...
                    foreach (var pred in result.predictions.OrderBy(p => PosKey(p.position)))
                    {
                        ...
                        foreach (var className in classColumns)
                        {
                            if (pred.probabilities != null && pred.probabilities.TryGetValue(className, out double prob))
                                row[className] = prob.ToString("P1"); // percentagem
                            else
                                row[className] = "";
                        }
                        dt.Rows.Add(row);
                ...
                //dataGridView_Predicitons.Dock = DockStyle.Fill;
                //dataGridView_Predicitons.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells;
                //dataGridView_Predicitons.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCells;
                //dataGridView_Predicitons.DataSource = dt;
                //dataGridView_Predicitons.AutoResizeColumns();
        ...
        #region Helpers para Z diferentes Grapes

        private void ToggleGrapeCaliberPanel()
        {
            bool anyGrapes = dataGridView_Table.Rows
                .Cast<DataGridViewRow>()
                .Where(r => !r.IsNewRow)
                .Select(r => r.Cells["Material"].Value as string)
                .Any(val => !string.IsNullOrWhiteSpace(val) &&
                            (val.Equals("Grapes", StringComparison.OrdinalIgnoreCase) ||
                             val.Equals("Uvas", StringComparison.OrdinalIgnoreCase)));

            panel_GrapeCaliber.Visible = anyGrapes;
        }

        private bool IsGrapesSelectedInTable()
        {
            return dataGridView_Table.Rows
                .Cast<DataGridViewRow>()
                .Where(r => !r.IsNewRow)
                .Select(r => r.Cells["Material"].Value as string)
                .Any(val => !string.IsNullOrWhiteSpace(val) &&
        ...
        private double CurrentGrapeZ()
        {
            // calibre 0 → Z=8; calibre 1 → Z=10; calibre 2 → Z=20
            int idx = comboBox_GrapeCaliber?.SelectedIndex ?? 1;
            return (idx == 0) ? 8.0 : (idx == 1) ? 10.0 : 20.0;
        }

        private static bool IsBetweenPos(string posName, int from, int to)
        {
            var m = System.Text.RegularExpressions.Regex.Match(posName ?? "", @"\d+");
        ...
        public class AnnotationDto
        {
            ...
            public object value { get; set; } //aceita string, double e/ou bool
        }
        ...
        public class SampleDto
        {
            public int id { get; set; }
            public string position { get; set; }
            public string submaterial_name { get; set; }
        ...
        public class BatchOption
        {
            ...
            public string Display => $"{created_at:yyyy-MM-dd HH:mm} | {Count} samples";
        }

        public void SetupDataGridView_Editor()
        {
            dataGridView_Editor.AllowUserToAddRows = false;
            //Clear any existing columns
            dataGridView_Editor.Columns.Clear();
            
            //1) Sample ID
            DataGridViewTextBoxColumn idCol = new DataGridViewTextBoxColumn();
            idCol.HeaderText = "Sample ID";
            idCol.Name = "SampleId";
            idCol.ReadOnly = true; //filled when click on Save button of positions in Scanner
            idCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.ColumnHeader;
            dataGridView_Editor.Columns.Add(idCol);
            ...
            positionCol.ReadOnly = true; //user can write here
            positionCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.ColumnHeader;
            dataGridView_Editor.Columns.Add(positionCol);

            //3) SubmMaterial column
            DataGridViewTextBoxColumn submatCol = new DataGridViewTextBoxColumn();
            submatCol.HeaderText = "Type";
            submatCol.Name = "Submaterial";
            submatCol.ReadOnly = true;
            submatCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.ColumnHeader;
            dataGridView_Editor.Columns.Add(submatCol);

            //4) Key Value column
            DataGridViewTextBoxColumn keyCol = new DataGridViewTextBoxColumn();
            keyCol.HeaderText = "Key Value"; // textBox_EditorKey?.Text ?? "Key Value";
            keyCol.Name = "KeyValue";
            keyCol.ReadOnly = true; //filled automatically
            keyCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.ColumnHeader;
            dataGridView_Editor.Columns.Add(keyCol);

            //5) Annotation column (editable)
            DataGridViewTextBoxColumn annotationCol = new DataGridViewTextBoxColumn();
            annotationCol.HeaderText = "Annotation";
            annotationCol.Name = "Annotation";
            annotationCol.ReadOnly = false; //user can write here
            annotationCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.ColumnHeader;
            dataGridView_Editor.Columns.Add(annotationCol);

            //6) Datetime "Created at" column
            DataGridViewTextBoxColumn createdCol = new DataGridViewTextBoxColumn();
            createdCol.HeaderText = "Created at";
            createdCol.Name = "Created_at";
            createdCol.ReadOnly = true; //filled automatically
            createdCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
            dataGridView_Editor.Columns.Add(createdCol);
        }

        private async Task Editor_StartAsync()
        {
            await Editor_LoadMaterialsAsync();   // 1) carrega materiais
            
            //Liga handlers dos botões
            button_EditorShow.Click += async (_, __) => await Editor_BindGridAsync(); 
            button_EditorSearch.Click += async (_, __) => await Editor_ReloadBatchesAsync();
            button_EditorSave.Click += async (_, __) => await Editor_SaveAnnotationsAsync();

            // Desliga handlers temporariamente (evita SelectedIndexChanged durante DataSource)
            comboBox_EditorMaterial.SelectedIndexChanged -= Combo_EditorMaterial_SelectedIndexChanged;
            comboBox_Batch.SelectedIndexChanged -= Combo_Batch_SelectedIndexChanged;
            //Liga handlers novamente
            comboBox_EditorMaterial.SelectedIndexChanged += Combo_EditorMaterial_SelectedIndexChanged;
            comboBox_Batch.SelectedIndexChanged += Combo_Batch_SelectedIndexChanged;

            /* Primeira carga (se houver seleção válida)
            if (comboBox_EditorMaterial.SelectedValue is int matId)
            {
                await Editor_ReloadBatchesAsync();
            }*/
        }

        private void Combo_EditorMaterial_SelectedIndexChanged(object sender, EventArgs e)
        {
            comboBox_Batch.DataSource = null;
            dataGridView_Editor.DataSource = null;
            dataGridView_Editor.Rows.Clear();
            dataGridView_Editor.Columns.Clear();
        }

        private void Combo_Batch_SelectedIndexChanged(object sender, EventArgs e)
        {
            dataGridView_Editor.DataSource = null;
            dataGridView_Editor.Rows.Clear();
            dataGridView_Editor.Columns.Clear();
        ...
        private async Task Editor_BindGridAsync()
        {
            dataGridView_Editor.DataSource = null;
            dataGridView_Editor.Rows.Clear();
            dataGridView_Editor.Columns.Clear();
            SetupDataGridView_Editor();

            if (!(comboBox_EditorMaterial.SelectedValue is int matId)) return;
            if (!(comboBox_Batch.SelectedValue is int batchId)) return;

            //var all = await _apiClient.GetFromJsonAsync<List<SampleDto>>($"api/samples/?material_id={matId}&sequence_id={batchId}")
            //?? new List<SampleDto>();

            // Faz o GET manualmente para poderes inspecionar o JSON
            var response = await _apiClient.GetAsync($"api/samples/?material_id={matId}&sequence_id={batchId}");
            var json = await response.Content.ReadAsStringAsync();

            // Debug → Mostra no Output Window ou até em MessageBox
            Console.WriteLine("DEBUG /api/samples response:");
            Console.WriteLine(json);

            // Se quiseres ver num popup
            // MessageBox.Show(json.Substring(0, Math.Min(json.Length, 1000)), "Payload (truncado)");

            var all = JsonConvert.DeserializeObject<List<SampleDto>>(json) ?? new List<SampleDto>();

            var key = (textBox_EditorKey?.Text ?? "").Trim();

            foreach (var s in all.OrderBy(s => PosKey(s.position)))
            {
                object val = ""; //valor da annotation para key escrita na textBox
                if (!string.IsNullOrWhiteSpace(key))
                {
                    var ann = (s.annotations ?? new List<AnnotationDto>())
                              .FirstOrDefault(a => string.Equals(a.key, key, StringComparison.OrdinalIgnoreCase));
                    if (ann != null)
                        val = ann.value ?? ann.value_text ??
                              (ann.value_number?.ToString(CultureInfo.InvariantCulture) ??
                               (ann.value_boolean?.ToString() ?? ""));
                }

                dataGridView_Editor.Rows.Add(
                    s.id,
                    s.position ?? "",
                    s.submaterial_name ?? "(sem submaterial)",       
                    key,
                    val,//annValue, //mostra valor se annotation já tiver sido atribuída (editável)
                    s.timestamp.ToString("yyyy-MM-dd HH:mm")
                );
            }
        }

        private async Task Editor_LoadMaterialsAsync()
        {
            var mats = await _apiClient.GetFromJsonAsync<List<MaterialDto>>("api/materials/");

            comboBox_EditorMaterial.SelectedIndexChanged -= Combo_EditorMaterial_SelectedIndexChanged;
            comboBox_EditorMaterial.DisplayMember = "Name"; //nameof(MaterialDto.Name)  
            comboBox_EditorMaterial.ValueMember = "Id";   // nameof(MaterialDto.Id)
            comboBox_EditorMaterial.DataSource = mats ?? new List<MaterialDto>();
            comboBox_EditorMaterial.SelectedIndexChanged += Combo_EditorMaterial_SelectedIndexChanged;

            if (comboBox_EditorMaterial.Items.Count > 0 && comboBox_EditorMaterial.SelectedIndex < 0)
                comboBox_EditorMaterial.SelectedIndex = 0;
        }

        private async Task Editor_ReloadBatchesAsync()
        {
            ProgressWindowStart("Search", "Searching all sequences, please wait...", false);
            ...
            // 1) Buscar samples do material 
            var samples = await _apiClient.GetFromJsonAsync<List<SampleDto>>($"api/samples/?material_id={matId}")
                           ?? new List<SampleDto>();

            // 2) Agrupar por sequence_id (batch) e ordenar por data mais recente
            var batches = samples
                .Where(s => s.sequence_id != 0)
                .GroupBy(s => s.sequence_id)
                .Select(g => new
                {
                    Id = g.Key,
                    Count = g.Count(),
                    Created = g.Max(s => s.timestamp),
                    Label = $"Seq {g.Key} · {g.Count()} samples · {g.Max(s => s.timestamp):yyyy-MM-dd HH:mm}"
                })
                .OrderByDescending(b => b.Created)
                .ToList();

            comboBox_Batch.DisplayMember = "Label";
            comboBox_Batch.ValueMember = "Id";
            comboBox_Batch.DataSource = batches;

            ProgressWindowCompleted();
        }

        private async Task Editor_SaveAnnotationsAsync()
        {
            ProgressWindowStart("Annotations", "Annotations being saved, please wait...", false);
            var key = (textBox_EditorKey?.Text ?? "").Trim();
            if (string.IsNullOrEmpty(key))
            {
                MessageBox.Show("Please enter a key before saving.");
                return;
            }

            foreach (DataGridViewRow row in dataGridView_Editor.Rows)
            {
                if (row.IsNewRow) continue;

                int sampleId = Convert.ToInt32(row.Cells["SampleId"].Value);
                var value = row.Cells["Annotation"].Value?.ToString();
                ...
                if (double.TryParse(value, NumberStyles.Any, CultureInfo.InvariantCulture, out var parsedNum))
                {
                    num = parsedNum;
                }
                ...
                string json = JsonConvert.SerializeObject(
                    payload,
                    new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore, Formatting = Newtonsoft.Json.Formatting.Indented }
                );
                Console.WriteLine($"Payload enviado: {json}");

                try
                {
                    var response = await _apiClient.PostAsync("editor/annotations/", new StringContent(json, Encoding.UTF8, "application/json"));
                    var respBody = await response.Content.ReadAsStringAsync();
                    if (!response.IsSuccessStatusCode)
                    {
                        MessageBox.Show(
                            $"Error on saving annotation to Sample {sampleId}.\n" +
                ...
                {
                    MessageBox.Show($"Error on saving annotation to Sample {sampleId}:\n{ex.Message}\n\nPayload:\n{json}");
                }
                finally
                {
                    ProgressWindowCompleted();
        ...
        private void dataGridView_Editor_KeyDown(object sender, KeyEventArgs e)
        {
            // If no cell is active, bail out
            if (dataGridView_Editor.CurrentCell == null) return;

            var cell = dataGridView_Editor.CurrentCell;
            int row = cell.RowIndex;
            int col = cell.ColumnIndex;
            string colName = dataGridView_Editor.Columns[col].Name;

            //
            // 1) Ctrl+F on “Annotation” column ⇒ fill downward with the same annotation
            //
            if (e.Control && e.KeyCode == Keys.F && colName == "Annotation")
            {
                string input = Interaction.InputBox(
                    "How many rows do you wish to fill?",
                    "Fill Downward",
                    "1"
                );
                ...
                if (int.TryParse(input, out int count) && count > 0)
                {
                    if (cell != null)
                    {
                        ...
                        for (int i = 1; i <= count && row + i < dataGridView_Editor.Rows.Count; i++)
                        {
                            var targetRow = dataGridView_Editor.Rows[row + i];
                            //if (targetRow.IsNewRow) break;
                            targetRow.Cells[col].Value = value;
                        }
                    }
                }
                e.Handled = true;
