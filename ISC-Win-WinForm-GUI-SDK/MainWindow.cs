...
namespace ISC_Win_WinForm_GUI
{
    public partial class MainWindow : Form
    {
        ...
        public void CommonUISetup(string[] args)
        {
            ...
            //(ver depois)Label_ContScan.Text = "";
            BackupFacRef_Msg = "";
            RestoreFacRef_Msg = "";
        ...
        /*private void PopulateScanRecord(ScanRecord rec)
        {
            ...
            // 3.2 Arrays de dados
            rec.Reference = Scan.ReferenceIntensity?.Select(v => (double)v).ToArray() ?? Array.Empty<double>();
            rec.Intensity = Scan.Intensity?.Select(v => (double)v).ToArray() ?? Array.Empty<double>();
            rec.Absorbance = Scan.Absorbance?.ToArray() ?? Array.Empty<double>();
            rec.Reflectance = Scan.Reflectance?.ToArray() ?? Array.Empty<double>();

            // 3.3 Min/Max wavelength do dispositivo
            rec.MinWavelength = Device.DevInfo.MinWavelength;
            rec.MaxWavelength = Device.DevInfo.MaxWavelength;

            // 3.4 Parâmetros extra
            //rec.Parameters = BuildParameters();
        }

        private async Task PushScanRecordAsync(ScanRecord record)
        {
            try
            {
                ...
                //var response = await _apiClient.PostAsync("api/scans/", content);
                var response = await _apiClient.PostAsync("upload-sample/", content);
                if (!response.IsSuccessStatusCode)
                {
                    MessageBox.Show(
                      $"API push failed for {record.Position}. Status: {response.StatusCode}",
            ...
            {
                MessageBox.Show(
                  $"API error for {record.Position}: {ex.Message}",
        ...
        private async Task PushUploadSampleAsync(UploadSample usp)
        {
            try
            {
                // Embrulha o usp num array de tamanho 1
                var payload = new[] { usp };
        ...
        public void SetupDataGridView()
        {
            ...
            //dataGridView_Table.AutoGenerateColumns = false;
        ...
        private void dataGridView_Table_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            //
            // 1) Ctrl+1 on “SampleName” column ⇒ fill downward with incrementing names
            //
            if (e.Control && e.KeyCode == Keys.D1 && colName == "SampleName")
            {
        ...
        public async Task<bool> ConfigurarPorta(string port)
        {
            try
            {
                ...
                serialPort = new SerialPort(port, 112500)
                {
                ...
                // Dar tempo ao hardware para inicializar — sem bloquear o thread!
